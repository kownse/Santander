...
def compiled_leak_result():
    ...
    for i in range(max_nlags):
        ...
        train_leak[c] = _get_leak(train, cols,l+ext, i)
...
train_res.to_csv('../input/train_leak_22.csv', index=False)
...
def compiled_leak_result_test(max_nlags):
    ...
    for i in range(max_nlags):
        ...
        print('Processing lag', i)
        test_leak[c] = _get_leak(test, cols, l+ext, i)
...
max_nlags = 38
test_leak, test_result = compiled_leak_result_test(max_nlags=max_nlags)
...
test_res.to_csv('../input/test_leak_22.csv', index=False)
...
sub.to_csv(f"../result/non_fake_sub_lag_max{best_lag}.csv", index=False)
